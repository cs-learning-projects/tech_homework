...
class Purchase_price_total(object):
    def product_price_calc(self, name, price, quantity, q_description):
        if q_description == 'unit(s)' and quantity >= 2:
            ...
            print(total_price, 'rubles (with 5% discount) for', quantity,
                  q_description, 'of', name)
            ...
            print(total_price, 'rubles for', quantity, q_description,
                  '(rounded to 50g) of', name)
        else:
            total_price = price * quantity
            print(total_price, 'rubles for', quantity, q_description, 'of',
                  name)  # как убрать пробел между суммой и rubles?
        return total_price
...
purchase_w1.product_price_calc(w1.name, w1.price, w1.weight,
                               w1.volume_descripton)

u1 = Product_by_unit('chips', 55, 2)
purchase_u1 = Purchase_price_total()
purchase_u1.product_price_calc(u1.name, u1.price, u1.unit,
                               u1.volume_descripton)

v1 = Product_by_volume('pepsi', 45, 3)
purchase_u1 = Purchase_price_total()
purchase_u1.product_price_calc(v1.name, v1.price, v1.volume,
                               v1.volume_descripton)
